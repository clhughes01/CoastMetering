...
export default function AdminProviderBillsPage() {
  ...
  const handleAddBill = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setForm({ property_id: "", account_number: "", billing_period_start: "", billing_period_end: "", amount_due: "", due_date: "", pdf_url: "" })
      setShowAdd(false)
      await loadBills()
    } catch (e) {
  ...
  return (
    <div className="flex flex-col min-h-screen">
      <Header title="Escondido / Provider Bills" breadcrumbs={[{ label: "Escondido Bills" }]} basePath={BASE} />
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="flex items-center gap-2 text-lg">
              ...
              Utility provider bills (Escondido Water)
            </CardTitle>
          ...
          <CardContent className="space-y-4">
            <p className="text-sm text-muted-foreground">
              Bills are fetched automatically from the Invoice Cloud portal on a schedule. If the scraper returns 0 bills, you can add a bill here using data from your paper/PDF bill.
            </p>
